...
class OakdTracker:
    ...
    def start_background(self, rate_hz: float = 60.0):
        ...
        def _loop():
            ...
            while not self._stop_evt.is_set():
                time_prof = np.append(time_prof,time.time()) 
                try:
                    vo = self.poll_object()
                    time_prof = np.append(time_prof,time.time()) 
                    if vo is not None:
                ...
                #print("Number of times: ", len(time_prof))
                #print("Cam Background Time cost: ", time_prof[1:]-time_prof[0:-1])
    ...
    def poll_object(self) -> Optional[VisionObject]:
        ...
        if (current_time - self.startTime) > 0.01:
