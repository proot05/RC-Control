...
class XboxController:
    ...
    def _norm_t(self, v, lo, hi, dz=0.02):
        c = (v - lo) / max(1, (hi - lo))
        x = max(0.0, min(1.0, c))
            ...
            for e in self.dev.read():
                if e.type == ecodes.EV_ABS:
                    if e.code == ecodes.ABS_X:
                        self.state.steer = self._norm(e.value, -32768, 32767)
                    elif e.code == ecodes.ABS_RZ:
                        self.state.accel = self._norm_t(e.value, 0, 255)
                    elif e.code == ecodes.ABS_Z:
                        self.state.brake = self._norm_t(e.value, 0, 255)
                    ...
                    elif e.code == ecodes.BTN_NORTH: # X button
